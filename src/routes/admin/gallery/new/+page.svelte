<script lang="ts">
	...
	function selectDate(day: number) {
		form.date = `${months[calMonth]} ${day}, ${calYear}`;
		showCalendar = false;
	}
	...
	onMount(async () => {
		...
		function onDocClick() { showCalendar = false; }
				...
				<div class="grid grid-cols-2 gap-4">
					...
					<div>
						...
						<div class="relative" onclick={(e) => e.stopPropagation()}>
							<input type="text" readonly bind:value={form.date} placeholder="Select date"
								...
								onclick={() => showCalendar = !showCalendar} onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { showCalendar = !showCalendar; } }} />
							{#if showCalendar}
									...
									<div class="grid grid-cols-7 gap-0.5 text-center text-xs">
										...
										{#each Array(daysInMonth) as _, i}
											...
											<button onclick={() => selectDate(day)}
												class="rounded-lg py-1 text-text transition hover:bg-primary hover:text-white {form.date === `${months[calMonth]} ${day}, ${calYear}` ? 'bg-primary text-white' : ''}"
