<script lang="ts">
	...
	onMount(async () => {
		try {
			items = await list<Course>('courses');
		} catch (e) {
...
<div class="flex flex-wrap items-center justify-between gap-4">
	...
	<button onclick={() => goto('/admin/courses/edit')} class="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white transition hover:bg-primary-hover">+ Add Course</button>
</div>
	...
	<table class="w-full text-left text-sm">
		...
		<tbody>
				...
				<tr class="cursor-pointer border-b border-border last:border-0 hover:bg-surface/50" onclick={() => selected = item}>
					...
					<td class="px-4 py-3" onclick={(e) => e.stopPropagation()}>
						<div class="flex gap-1">
							<button onclick={() => goto('/admin/courses/edit?id=' + item.id)} class="rounded p-1.5 text-text-muted transition hover:bg-background hover:text-primary"><Edit3 size={15} /></button>
			...
			{#if filtered.length === 0}
				<tr><td colspan="5" class="px-4 py-8 text-center text-text-muted">No courses found</td></tr>
			{/if}
		</tbody>
	</table>
</div>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onclick={() => selected = null}>
		<div class="mx-4 w-full max-w-md rounded-xl border border-border bg-surface shadow-lg" onclick={(e) => e.stopPropagation()}>
			...
			<div class="grid grid-cols-2 gap-2 border-t border-border px-5 py-4">
				<button onclick={() => { const s = selected; selected = null; goto('/admin/courses/edit?id=' + s!.id); }}
